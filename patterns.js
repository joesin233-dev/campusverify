...
/* -------------------------------------------------------------------------
   CRAWLER EXTRACTION RULES — new in v2
   Used by api/crawler.js to pull structured facts out of an institution's
   raw page text. Zambian mobile format (+260 7XXXXXXXX / 07XXXXXXXX),
   ...
   ------------------------------------------------------------------------- */
/* Phone numbers in the wild look like: 0960871744, 0960 871 744,
   +260 960 871 744, 260960871744. One regex over raw page text still
   finds the digits fine — the real fix is NORMALIZING before any
   comparison happens, so "0960 871 744" (from a website) correctly
   matches "+260960871744" (typed by a student). normalisePhone() below
   is the single source of truth for that — used both when extracting
   from crawled pages and when comparing a student's input. */
...
function normalisePhone(value) {
  ...
  return digits; // bare 9-digit local number, e.g. "960871744"
}
...
/* -------------------------------------------------------------------------
   PAYMENT-DETAIL EXTRACTION — new in v3.1
   A bank NAME alone ("Zanaco") being on a page proves almost nothing —
   most institutions mention their bank without publishing account
   numbers. These extractors look for account numbers, account names,
   and branch details only when they appear near an explicit label
   ("account number:", "account name:", "branch:") so we don't grab
   random long digit strings (phone numbers, dates, reference codes).
   ------------------------------------------------------------------------- */
const BANK_ACCOUNT_REGEX = /(?:account\s*(?:no\.?|number|#)\s*[:\-]?\s*)(\d{8,16})/gi;
const ACCOUNT_NAME_REGEX = /(?:account\s*name\s*[:\-]?\s*)([A-Za-z][A-Za-z .&'\-]{2,60}?)(?=[.\n<]|(?:account\s*(?:no\.?|number)|branch)\b|$)/gi;
const BRANCH_REGEX = /(?:branch\s*(?:name)?\s*[:\-]?\s*)([A-Za-z][A-Za-z .&'\-]{2,60}?)(?=[.\n<]|(?:account\s*(?:no\.?|number)|branch)\b|$)/gi;
...
function extractBankAccounts(text) {
  ...
  while ((m = BANK_ACCOUNT_REGEX.exec(text)) !== null) results.push(m[1].trim());
  return Array.from(new Set(results));
}
function extractAccountNames(text) {
  var results = [], m;
  ACCOUNT_NAME_REGEX.lastIndex = 0;
  while ((m = ACCOUNT_NAME_REGEX.exec(text)) !== null) results.push(m[1].trim().replace(/\s+/g, " "));
  return Array.from(new Set(results));
}
function extractBranches(text) {
  var results = [], m;
  BRANCH_REGEX.lastIndex = 0;
  while ((m = BRANCH_REGEX.exec(text)) !== null) results.push(m[1].trim().replace(/\s+/g, " "));
  return Array.from(new Set(results));
}
function extractPaymentInstructions(text) {
  ...
  PAYMENT_INSTRUCTION_TRIGGERS.forEach(function (trigger) {
    ...
    if (idx !== -1) {
      // Grab a short surrounding snippet, not the whole page — enough
      // context to be useful, short enough to stay a "snippet" not a
      // reproduction of page content.
